...
LOG_CONFIG="$PARENT/conf/log4j2.xml"

LIB_PATH="$PARENT/server/webapp/metaspace/WEB-INF/lib"
for i in "$LIB_PATH/"*.jar; do
  ATLASCPPATH="${ATLASCPPATH}:$i"
done
LIB_PATH="$PARENT/"
# Construct Atlas classpath.
for i in "$LIB_PATH/hook/impala/atlas-impala-plugin-impl/"*.jar; do
  ATLASCPPATH="${ATLASCPPATH}:$i"
done
...
JAVA_PROPERTIES="$ATLAS_OPTS -Datlas.conf=$PARENT/conf -Datlas -Datlas.log.dir=$ATLAS_LOG_DIR -Datlas.log.file=$ATLAS_LOG_FILE -Dlog4j.configuration=file://$LOG_CONFIG"
