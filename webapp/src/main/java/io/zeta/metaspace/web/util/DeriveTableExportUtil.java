...
public class DeriveTableExportUtil {
    ...
    public static File deriveTableExport(String tableName){
        File classPathResource = new File(tableName);
    ...
    public static List<SourceInfoDeriveColumnDTO> getPojo(List<SourceInfoDeriveColumnVO> sourceInfoDeriveColumnVOS){
        ...
        for (SourceInfoDeriveColumnVO vo1:sourceInfoDeriveColumnVOS) {
            ...
            sourceInfoDeriveColumnDTO.setGroupField(getTureOrFalse(vo1.isGroupField()));
            sourceInfoDeriveColumnDTO.setRemoveSensitive(getTureOrFalse(vo1.isRemoveSensitive()));
            sourceInfoDeriveColumnDTO.setImportant(getTureOrFalse(vo1.isImportant()));
            sourceInfoDeriveColumnDTO.setPermissionField(getTureOrFalse(vo1.isPermissionField()));
            sourceInfoDeriveColumnDTO.setSecret(getTureOrFalse(vo1.isSecret()));
            sourceInfoDeriveColumnDTO.setPrimaryKey(getTureOrFalse(vo1.isPrimaryKey()));
            list.add(sourceInfoDeriveColumnDTO);
        }
        return list;
    }

    private static String getTureOrFalse(boolean f){
        if(f){
            return "是";
        }else {
            return "否";
