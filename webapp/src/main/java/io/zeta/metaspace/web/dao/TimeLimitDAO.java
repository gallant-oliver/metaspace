...
public interface TimeLimitDAO {
    ...
    @Insert({" insert into timelimit(id,name,desc,type,start_time,end_time,creator,updater,state,version,delete,create_time,tenantId) ",
            " values(#{timeLimit.id},#{timeLimit.name},#{timeLimit.desc},#{timeLimit.type},#{timeLimit.startTime},#{timeLimit.endTime},#{timeLimit.creator},#{timeLimit.updater},#{timeLimit.state},(select (case when max(version) is null then 0 else max(version) end)+1 from timelimit where tenantid=#{tenantId}),#{timeLimit.delete},#{timeLimit.create_time},#{tenantId},)"})
    public int addTimeLimit(@Param("task") TimelimitEntity timeLimit, @Param("tenantId") String tenantId);


    @Select({" select id from timelimit where tenantid=#{tenantId} and name=#{timeLimitName}"})
    public List<String> getTimeLimitByName(@Param("timeLimitName") String timeLimitName, @Param("tenantId") String tenantId);
