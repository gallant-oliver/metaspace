...
public class PreviewUploadXlsServiceImpl implements PreviewUploadService {
    @Override
    public io.zeta.metaspace.web.model.filetable.Workbook parser(String filePath) throws IOException, ExcelException {
        Workbook workbook = FastExcel.createReadableWorkbook(new File(filePath));
        io.zeta.metaspace.web.model.filetable.Workbook gWorkbook = new io.zeta.metaspace.web.model.filetable.Workbook();
        try {
            workbook.open();
            if (workbook != null) {
    ...
    private UploadPreview getUploadPreview(String jobId, UploadConfig uploadConfig, int size) {
        try {
            ...
            io.zeta.metaspace.web.model.filetable.Workbook workbook = uploadFileCache.get(jobId);
