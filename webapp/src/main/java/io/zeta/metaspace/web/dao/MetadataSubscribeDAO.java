...
public interface MetadataSubscribeDAO {
    ...
    @Select("select count(*) from metadata_subscribe where table_guid=#{tableGuid} and user_id=#{userId}")
    public int existSubscribe(@Param("tableGuid")String tableGuid, @Param("userId")String userId);
}
