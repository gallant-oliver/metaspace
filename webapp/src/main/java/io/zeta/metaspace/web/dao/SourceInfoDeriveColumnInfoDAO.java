...
public interface SourceInfoDeriveColumnInfoDAO {
    ...
    @Insert({"<script>",
            " INSERT INTO source_info_derive_column_info(id, column_guid, column_name_en, column_name_zh, data_type, source_table_guid, ",
            " source_column_guid, primary_key, remove_sensitive, mapping_rule, mapping_describe, group_field, permission_field, ",
            " secret, secret_period, important, remark, tags, desensitization_rules, custom, tenant_id, table_guid, sort) ",
            " VALUES (#{deriveColumn.id},#{deriveColumn.columnGuid},#{deriveColumn.columnNameEn},#{deriveColumn.columnNameZh},",
            " #{deriveColumn.dataType},#{deriveColumn.sourceTableGuid},#{deriveColumn.sourceColumnGuid},#{deriveColumn.primaryKey},#{deriveColumn.removeSensitive},",
            " #{deriveColumn.mappingRule},#{deriveColumn.mappingDescribe},#{deriveColumn.groupField},#{deriveColumn.permissionField},",
            " #{deriveColumn.secret},#{deriveColumn.secretPeriod},#{deriveColumn.important},#{deriveColumn.remark},#{deriveColumn.tags},#{deriveColumn.desensitizationRules},#{deriveColumn.custom},#{deriveColumn.tenantId},",
    ...
    @Insert({"<script>",
            ...
            " source_table_guid = excluded.source_table_guid ,source_column_guid = excluded.source_column_guid ,primary_key = excluded.primary_key ,remove_sensitive = excluded.remove_sensitive ,mapping_rule = excluded.mapping_rule,",
            ...
            " remark = excluded.remark,tags = excluded.tags,desensitization_rules = excluded.desensitization_rules,custom = excluded.custom,tenant_id = excluded.tenant_id,table_guid= excluded.table_guid,sort= excluded.sort",
            "</script>"})
