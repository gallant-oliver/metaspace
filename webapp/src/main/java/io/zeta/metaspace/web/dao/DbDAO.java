...
public interface DbDAO {
    ...
    @Select({"<script>",
                ...
                "from  data_source t2 join data_source t3 ",
                "on (t2.source_type = t3.source_type and t2.ip = t3.ip and t2.port = t3.port and t2.source_id = #{datasourceId} and t3.source_id != #{datasourceId})",
                "join source_db t4 on t3.source_id = t4.source_id ",
                "join db_info t5 on t4.db_guid = t5.database_guid ",
                "<if test='dbNames != null and dbNames.size()>0' >",
                    "where t5.database_name in ",
