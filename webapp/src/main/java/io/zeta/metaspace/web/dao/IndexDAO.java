...
public interface IndexDAO {
    ...
    @Select({"<script>" ,
            ...
            " </if>",
            " left join category ca on T.index_field_id=ca.guid " ,
            ...
            " where T.tenant_id=#{tenantId} and ca.categorytype=#{categoryType} and ca.tenantid=#{tenantId} and T.rn=1 and T.index_field_id in  ",
            " <foreach item='indexFieldId' index='index' collection='pageQueryDTO.indexFieldIds' separator=',' open='(' close=')'>",
            " #{indexFieldId} ",
            " </foreach> ",
