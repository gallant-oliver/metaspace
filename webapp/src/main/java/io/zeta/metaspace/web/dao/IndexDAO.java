...
public interface IndexDAO {
    ...
    @Select({"<script>" ,
            ...
            " #{indexFieldId} ",
            " ) ",
            " select *,bl.username as businessLeaderName, c.username as creatorName,u.username as updaterName,ca.name as indexFieldName from T ",
            " </if>",
            " left join category ca on iai.index_field_id=ca.guid " ,
            " left join users bl on T.business_leader=bl.userid " ,
            " left join users c on iai.creator=c.userid " ,
            " left join users u on iai.updater=u.userid " ,
            " where T.tenant_id=#{tenantId} and ca.categorytype=#{categoryType} and ca.tenantid=#{tenantId} and index_field_id in  ",
            " <foreach item='indexFieldId' index='index' collection='pageQueryDTO.indexFieldIds' separator=',' open='(' close=')'>",
            " #{indexFieldId} ",
