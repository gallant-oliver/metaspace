...
public interface IndexDAO {
    ...
    @Select({"<script>",
            "SELECT index_id,index_name,index_identification FROM index_atomic_info WHERE tenant_id = #{tenantId} AND index_name in ",
            " <foreach item='item' index='index' collection='nameList' separator=',' open='(' close=')'>",
            " #{item} ",
            " </foreach>",
            " UNION SELECT index_id,index_name,index_identification FROM index_atomic_info WHERE tenant_id = #{tenantId} AND index_identification in ",
