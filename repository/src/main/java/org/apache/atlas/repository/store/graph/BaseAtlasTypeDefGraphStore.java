 ...
 */
public abstract class BaseAtlasTypeDefGraphStore implements AtlasTypeDefStore {

    private static final Logger LOG = LoggerFactory.getLogger(BaseAtlasTypeDefGraphStore.class);

    private final AtlasTypeRegistry          typeRegistry;
    private final Set<TypeDefChangeListener> typeDefChangeListeners;
    private final int                        typeUpdateLockMaxWaitTimeSeconds;

    protected BaseAtlasTypeDefGraphStore(AtlasTypeRegistry typeRegistry,
    ...
    private static ExecutorService executor = new ThreadPoolExecutor(5, 5, 0L, TimeUnit.MILLISECONDS, new LinkedBlockingQueue<>(), new ThreadFactoryBuilder().setNameFormat("AtlasTypeDefGraphStore" + " thread-%d").build());
    ...
    @Override
    public BaseAtlasBaseTypeDef getByName(String name) throws AtlasBaseException {
        if (StringUtils.isBlank(name)) {
            throw new AtlasBaseException(AtlasErrorCode.TYPE_NAME_INVALID, "", name);
        }
        BaseAtlasType type = typeRegistry.getType(name);
        return getTypeDefFromType(type);
    }

    @Override
    public BaseAtlasBaseTypeDef getByGuid(String guid) throws AtlasBaseException {
    ...
    private BaseAtlasBaseTypeDef getTypeDefFromType(BaseAtlasType type) throws AtlasBaseException {
        BaseAtlasBaseTypeDef ret;
    ...
    private <T extends BaseAtlasBaseTypeDef> void removeDuplicateTypeIfAny(List<T> defList) {
    ...
    private class TypeRegistryUpdateHook extends GraphTransactionInterceptor.AbstractPostTransactionHook {
    ...
    private void tryUpdateByName(String name, BaseAtlasBaseTypeDef typeDef, AtlasTransientTypeRegistry ttr) throws AtlasBaseException {
        try {
    ...
    private void tryUpdateByGUID(String guid, BaseAtlasBaseTypeDef typeDef, AtlasTransientTypeRegistry ttr) throws AtlasBaseException {
        try {
