...
public abstract class AbstractAtlasBaseClient {
    ...
    private static final Logger LOG = LoggerFactory.getLogger(AbstractAtlasBaseClient.class);
    ...
    protected AbstractAtlasBaseClient() {
    }

    protected AbstractAtlasBaseClient(String[] baseUrl, String[] basicAuthUserNamePassword) {
    ...
    protected AbstractAtlasBaseClient(String... baseUrls) throws AtlasException {
        this(getCurrentUGI(), baseUrls);
    }

    protected AbstractAtlasBaseClient(UserGroupInformation ugi, String[] baseUrls) {
        this(ugi, ugi.getShortUserName(), baseUrls);
    }

    protected AbstractAtlasBaseClient(UserGroupInformation ugi, String doAsUser, String[] baseUrls) {
        initializeState(baseUrls, ugi, doAsUser);
    }

    protected AbstractAtlasBaseClient(String[] baseUrls, Cookie cookie) {
        this.cookie = cookie;
        initializeState(baseUrls, null, null);
    }

    @VisibleForTesting
    protected AbstractAtlasBaseClient(WebResource service, Configuration configuration) {
        this.service = service;
        this.configuration = configuration;
    }

    @VisibleForTesting
    protected AbstractAtlasBaseClient(Configuration configuration, String[] baseUrl, String[] basicAuthUserNamePassword) {
        if (basicAuthUserNamePassword != null) {
            if (basicAuthUserNamePassword.length > 0) {
                this.basicAuthUser = basicAuthUserNamePassword[0];
            }
    ...
    public String getAdminStatus() throws AtlasServiceException {
        String      result    = AbstractAtlasBaseClient.UNKNOWN_STATUS;
    ...
    int getNumberOfRetries() {
        return configuration.getInt(AbstractAtlasBaseClient.ATLAS_CLIENT_HA_RETRIES_KEY, AbstractAtlasBaseClient.DEFAULT_NUM_RETRIES);
    }
    ...
    public AtlasImportResult importData(AtlasImportRequest request, String absoluteFilePath) throws AtlasServiceException {
        ...
        MultiPart multipartEntity = new FormDataMultiPart()
                .field("request", BaseAtlasType.toJson(request), MediaType.APPLICATION_JSON_TYPE)
                .bodyPart(filePart);
    ...
    private int getSleepBetweenRetriesMs() {
        return configuration.getInt(AbstractAtlasBaseClient.ATLAS_CLIENT_HA_SLEEP_INTERVAL_MS_KEY, AbstractAtlasBaseClient.DEFAULT_SLEEP_BETWEEN_RETRIES_MS);
    }
