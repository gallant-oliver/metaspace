...
@Slf4j
public class OscarAdapterSource extends AbstractAdapterSource {

    private static final String JDBC_PREFIX = "jdbc:oscar://";

    public OscarAdapterSource(Adapter adapter, DataSourceInfo dataSourceInfo, DataSourcePool dataSourcePool) {
    ...
    public DataSource initDataSource() {
        try {
            ...
            jdbcConfig.setMinimumIdle(dataSourcePool != null ? dataSourcePool.getMinIdleSize() : 2);
    ...
    public Connection getConnectionForDriver() {
        try {
            Connection con = DriverManager.getConnection(getJdbcUrl(), getDataSourceInfo().getUserName(), getDataSourceInfo().getPassword());
            OscarJdbc2Connection oscarCon = (OscarJdbc2Connection)con;
            oscarCon.setClientEncoding("BIG5");
            return oscarCon;
    ...
    public AdapterExecutor getNewAdapterExecutor() {
        return new OscarAdapterExecutor(this);
    }
}
