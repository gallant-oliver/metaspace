...
public enum WrongSourceTypeSql {
    ...
    // ORACLE类型获取异常数据sql
    ORACEL_WRONG_SQL("ORACLE", "select t.table_name from all_tab_comments c right join all_tables t on c.table_name = t.table_name and c.owner = t.owner and c.table_type = 'TABLE' where (c.comments is null or c.comments = '') and t.owner = '%s'"),
